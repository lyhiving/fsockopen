...
class fsockopen{
	...
	//浏览器
	public static $UserAgent = 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/57.0.2987.133 Safari/537.36';
	public static $AcceptEncoding = ''; //压缩编码 gzip, deflate, sdch
	public static $AcceptLanguage = 'zh-CN,zh;q=0.8'; //语言
	public static $Connection = 'Close'; //持久连接 keep-alive 关闭 Close
	public static $HttpVersion = 'HTTP/1.1'; //使用HTTP 1.0协议，服务器会主动放弃chunked编码
	public static $OnContinue = false; // 关闭 HTTP/1.1 100 Continue 返回
	...
	public function POST($Url, $Content, $Referer='', $Cookie='', $ContentType=''){
		...
		$header = "POST ".$path." ".self::$HttpVersion."\r\n";
		...
		$header .= "Accept: */ *\r\n";
		if(self::$OnContinue === false){ //防止返回 HTTP/1.1 100 Continue
			$header .= "Expect:\r\n";
		}
		if(!empty($Referer)){
			$header .= "Referer: ".$Referer."\r\n"; //来源
		}
		if(!empty(self::$AcceptEncoding)){
			$header .= "Accept-Encoding: ".self::$AcceptEncoding."\r\n"; //压缩编码
		}
		$header .= "Accept-Language: ".self::$AcceptLanguage."\r\n"; //语言
		if(!empty($Cookie)){
	...
	public function GET($Url, $Referer='', $Cookie=''){
		list($fp, $path, $purl) = $this->NewChannel($Url);
		$header = "GET ".$path." ".self::$HttpVersion."\r\n";
		$header .= "Host: ".$purl['host']."\r\n";
		$header .= "Connection: ".self::$Connection."\r\n"; //持久连接
		$header .= "User-Agent: ".self::$UserAgent."\r\n"; //浏览器
		$header .= "Accept: */*\r\n";
